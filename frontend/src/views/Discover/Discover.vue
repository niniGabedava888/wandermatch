<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-5xl mx-auto px-6 py-6">
      <!-- Result count -->
      <div
        v-if="discover.searched && !discover.loading"
        class="flex items-center justify-between mb-5"
      >
        <p class="text-sm text-gray-500">
          <span class="font-semibold text-gray-800"
            >{{ discover.meta?.total ?? 0 }} travellers</span
          >
          {{ hasActiveFilters ? 'found' : 'exploring' }}
      ...
      <div v-if="discover.loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="i in 6"
          :key="i"
          class="bg-white border border-gray-200 rounded-2xl p-4 animate-pulse"
        >
      ...
      <!-- Results -->
      <div
        v-else-if="discover.results.length > 0"
      ...
      <!-- Pagination — inside the main div -->
      <DiscoveryPagination
        v-if="discover.meta && discover.meta.totalPages > 1"
        :meta="discover.meta"
...
import DiscoveryPagination from '../../components/discover/DiscoveryPagination.vue'

const discover = useDiscoverStore()
const selectedUser = ref<DiscoverResult | null>(null)

const hasActiveFilters = computed(() =>
  Object.values(discover.currentFilters).some((v) => v !== '' && v !== undefined),
)
