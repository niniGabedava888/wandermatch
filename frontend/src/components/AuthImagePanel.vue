<template>
  <div class="relative hidden md:flex flex-col w-1/2 overflow-hidden rounded-r-2xl">
    ...
    <div class="absolute bottom-0 left-0 right-0 p-8 text-white">
      <p class="text-sm leading-relaxed italic mb-4 opacity-90">
        "{{ currentTestimonial.quote }}"
      </p>
      <div class="flex items-center justify-between">
        <div>
          <p class="font-semibold text-sm">{{ currentTestimonial.name }}</p>
          <p class="text-xs opacity-70">{{ currentTestimonial.role }} · {{ currentTestimonial.location }}</p>
        </div>
...
const testimonials = [
  {
    quote: "WanderMatch helped me find the perfect travel companion for my solo trip to Japan. Couldn't have done it without this platform!",
    name: "Sarah Mitchell",
    role: "Solo Traveller",
  ...
  {
    quote: "I was nervous about travelling alone but WanderMatch connected me with amazing people who share my love for adventure.",
    name: "Carlos Rivera",
...
function next() { current.value = (current.value + 1) % testimonials.length }
function prev() { current.value = (current.value - 1 + testimonials.length) % testimonials.length }
</script>